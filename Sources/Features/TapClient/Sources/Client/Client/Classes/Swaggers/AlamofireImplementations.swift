...
class IoTizeTapRequestBuilderFactory: RequestBuilderFactory {
    func getNonDecodableBuilder<T>() -> RequestBuilder<T>.Type {
        return IoTizeTapRequestBuilder<T>.self
    }

    func getBuilder<T:Decodable>() -> RequestBuilder<T>.Type {
        return IoTizeTapDecodableRequestBuilder<T>.self
    }
}

// Store manager to retain its reference
private var managerStore: [String: IoTizeTap.SessionManager] = [:]

open class IoTizeTapRequestBuilder<T>: RequestBuilder<T> {
    ...
    /**
     ...
     configuration.
     */
    open func createSessionManager() -> IoTizeTap.SessionManager {
        let configuration = URLSessionConfiguration.default
        configuration.httpAdditionalHeaders = buildHeaders()
        return IoTizeTap.SessionManager(configuration: configuration)
    }
    ...
    override open func execute(_ completion: @escaping (_ response: Response<T>?, _ error: Error?) -> Void) {
        ...
        let xMethod = IoTizeTap.HTTPMethod(rawValue: method)
...
public enum IoTizeTapDecodableRequestBuilderError: Error {
...
open class IoTizeTapDecodableRequestBuilder<T:Decodable>: IoTizeTapRequestBuilder<T> {
        ...
        default:
            validatedRequest.responseData(completionHandler: { (dataResponse: DataResponse<Data>) in
                ...
                guard let data = dataResponse.data, !data.isEmpty else {
                    completion(nil, ErrorResponse.error(-1, nil, IoTizeTapDecodableRequestBuilderError.emptyDataResponse))
                    return
                }

                guard let httpResponse = dataResponse.response else {
                    completion(nil, ErrorResponse.error(-2, nil, IoTizeTapDecodableRequestBuilderError.nilHTTPResponse))
                    return
                }
